...
function cambioPelicula(posicionACambiar) {
   peliculaMayuscula = peliculas[posicionACambiar].toUpperCase()

  
  peliculas.unshift(peliculaMayuscula)
  
  peliculas.pop(posicionACambiar)
...
let eliminarJuego = peliculasAEstrenar.shift()

console.log(peliculasAEstrenar);


function juntarPeliculas(peliculas, peliculasAEstrenar) {
  return peliculas.concat(peliculasAEstrenar);
}

let peliculasFinal = juntarPeliculas(peliculas, peliculasAEstrenar);
